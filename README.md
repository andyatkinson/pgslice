...
## More on andyatkinson fork

Focused only on Declarative partitioning, Ruby on Rails 6.1+, PostgreSQL 13+.


### Default Partition (Postgres 11+)

Ideally, local development, and test databases, match production. Having partitioned tables in production with equivalent tables that are unpartitioned in development, misses the opportunity to test and fix query issues.

When rows are mostly append only, this is less critical. Active Record does not have explicit support for partitioned tables. DQL (Select) queries like ".first" and ".last" that depend on PRIMARY KEY `id` columns don't work.

DML modifications, including inserts, updates, and deletes, both single row inserts, and multi-row inserts (e.g. `insert_all` and `upsert_all`) also likely require changing your Active Record application code for partitioned table compatibility.

Various options were considered, between not partitioning at all, and having an excessive amount of partitions in development and test.

In lieu of that, Postgres 11+ provides a `DEFAULT` partition concept, and that was selected.

The default partition facilitates consistency between dev and prod, in partition table management, without excessive partitions, or historical or future partition creation requirements for dev and test.

The approach is:

1. Add the `DEFAULT` partition everywhere
1. Use pgslice to incrementally move every environment over to a partition table
1. For dev and test, leave the default partition. For prod, use the "retirement" concept from pgslice, and retire the default partition, replacing it with partitions per the strategy.

Our default strategy is to retain 2 years of data, and thus monthly `RANGE` partitions that go back 2 years.


### Unique Constraint and Rails `insert_all`

The SQL generated by Rails' `insert_all` includes an `ON CONFLICT` fragment, and depends on the table having a `UNIQUE` index covering one or more fields describining uniqueness.

By default this will be the `id` field, however for partitioned tables, with a `created_at` partition key column, a unique index cannot be defined solely on the `id` column.

Create a `UNIQUE` index covering both `(id, created_at)`.

A sample Rails migration may be generated to add the unique index.

### Implementation scripts

* Wrapped primary step by step methods of `pgslice`, with implementation-specific caller arguments
* Invoke the wrapper commands and handle small differences, like different schemas between different production databases
* Invoke this command as a scheduled job
*  Procedural top to bottom script of each command invocation, that can be run in dev, test, and production
